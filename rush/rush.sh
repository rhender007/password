...
# SUPERUNICORN_RULE="/mnt/c/Tools/hashcat/rules/SuperUnicorn.rule"
...
# WORD_LIST_CLEAN_LESS_THAN_9="${INPUT_DIR}/wordlist_clean_less_than_9.txt"
...
hashcat -m 500 -a 0 -O -S --status --status-timer=10 "$HASHES_FILE" -r "$TOGGLES_RULE" -r "$INCISIVE_LEET_RULE" "$WORD_LIST_CLEAN_LESS_THAN_10" | tee -a "$log_file"

# havent figured out the lessthan 9 logic yet. dont thin exclamation rule is correct
# awk 'length < 9' "$WORD_LIST_CLEAN_LESS_THAN_10" > "$WORD_LIST_CLEAN_LESS_THAN_9" | tee -a "$log_file"
# hashcat -m 500 -a 0 -O -S --status --status-timer=10 "$HASHES_FILE" -r "$TOGGLES_RULE" -r "$INCISIVE_LEET_RULE" "$WORD_LIST_CLEAN_LESS_THAN_9" -r "$EXCLAMATION_END_RULE" | tee -a "$log_file"
